 ...
 * - Analyse locale par patterns (rapide, fonctionne offline)
 * - API IA externe optionnelle (OpenAI/Claude si configure)
 */
...
$debug_info = [
    ...
    'sender_param' => $sender,
    'pdo_ok' => isset($pdo) && $pdo !== null
];

if ($reservation_id) {
    try {
        $stmt = $pdo->prepare("
            SELECT r.*, ll.nom_du_logement, ll.adresse, ll.ville_id,
                   v.nom as ville_nom
            FROM reservation r
        ...
        $debug_info['logement_id'] = $reservation['logement_id'] ?? null;
        $debug_info['ville_id_from_query'] = $reservation['ville_id'] ?? null;
        $debug_info['ville_nom_from_query'] = $reservation['ville_nom'] ?? null;

        if ($reservation && $reservation['logement_id']) {
            // Charger les equipements du logement
            $stmt = $pdo->prepare("SELECT * FROM logement_equipements WHERE logement_id = ?");
            $stmt->execute([$reservation['logement_id']]);
            $equipements = $stmt->fetch(PDO::FETCH_ASSOC);

            // Recuperer la ville du logement via ville_id
            $ville_id = $reservation['ville_id'] ?? null;
            $ville_logement = $reservation['ville_nom'] ?? null;
        }
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT r.*, ll.nom_du_logement, ll.adresse, ll.ville_id, ll.id as logement_id,
                   v.nom as ville_nom
            FROM reservation r
            LEFT JOIN liste_logements ll ON r.logement_id = ll.id
            LEFT JOIN villes v ON ll.ville_id = v.id
            WHERE (r.telephone = ? OR r.telephone = ?)
        ...
        if ($reservation && $reservation['logement_id']) {
            ...
            // Recuperer la ville du logement
            $ville_id = $reservation['ville_id'] ?? null;
            $ville_logement = $reservation['ville_nom'] ?? null;
        }
    } catch (PDOException $e) {
...
// Methode 1: via ville_id (si configure)
// Methode 2: via le nom de ville (fallback)
$recs = [];

if ($ville_id) {
    // Recherche par ville_id
    try {
        $stmt = $pdo->prepare("
    ...
    // Fallback: recherche par nom de ville (si ville_id pas configure)
    try {
        // D'abord trouver l'ID de la ville par son nom
        $stmt = $pdo->prepare("SELECT id FROM villes WHERE LOWER(nom) = LOWER(?) LIMIT 1");
...
if (!empty($recs)) {
    // La table utilise: 'partenaire', 'restaurant', 'activite'
    // Le code attend: 'restaurants', 'activites', 'partenaires'
    $category_map = [
...
$patterns = [
    // WiFi
    [
        'keywords' => ['wifi', 'wi-fi', 'internet', 'code wifi', 'mot de passe wifi', 'password wifi', 'connexion'],
        'category' => 'wifi',
        'priority' => 10
    ],
    // Arrivee / Check-in
    [
        'keywords' => ['arrivee', 'arriver', 'check-in', 'checkin', 'heure arrivee', 'quelle heure', 'entrer', 'rentrer', 'acces', 'cle', 'clef', 'boite a cle', 'digicode', 'code porte', 'code entree'],
        'category' => 'checkin',
        'priority' => 9
    ],
    // Depart / Check-out
    [
        'keywords' => ['depart', 'partir', 'check-out', 'checkout', 'heure depart', 'quitter', 'rendre cle', 'fin sejour'],
        'category' => 'checkout',
        'priority' => 9
    ],
    // Parking
    [
        'keywords' => ['parking', 'garer', 'voiture', 'stationnement', 'place', 'garage'],
        'category' => 'parking',
        'priority' => 8
    ],
    // Menage / Linge
    [
        'keywords' => ['menage', 'draps', 'serviettes', 'linge', 'propre', 'nettoyer', 'machine a laver', 'lave-linge'],
        'category' => 'menage',
        'priority' => 7
    ],
    // Equipements cuisine
    [
        'keywords' => ['cafe', 'cafetiere', 'nespresso', 'bouilloire', 'micro-onde', 'four', 'cuisine', 'ustensile', 'casserole', 'poele'],
        'category' => 'cuisine',
        'priority' => 6
    ],
    // TV / Divertissement
    [
        'keywords' => ['tv', 'television', 'tele', 'netflix', 'chaine', 'telecommande', 'amazon', 'disney'],
        'category' => 'tv',
        'priority' => 6
    ],
    // Chauffage / Climatisation
    [
        'keywords' => ['chauffage', 'chaud', 'froid', 'climatisation', 'clim', 'temperature', 'radiateur', 'thermostat'],
        'category' => 'chauffage',
        'priority' => 7
    ],
    // Probleme / Urgence
    [
        'keywords' => ['probleme', 'panne', 'casse', 'fonctionne pas', 'marche pas', 'urgence', 'aide', 'bloque', 'coincé'],
        'category' => 'probleme',
        'priority' => 10
    ],
    // Remerciement
    [
        'keywords' => ['merci', 'super', 'genial', 'parfait', 'excellent', 'bien passe', 'satisfait'],
        'category' => 'remerciement',
        'priority' => 3
    ],
    // Retard
    [
        'keywords' => ['retard', 'tard', 'arriver plus tard', 'decaler', 'reporter'],
        'category' => 'retard',
        'priority' => 8
    ],
    // Adresse / Localisation
    [
        'keywords' => ['adresse', 'ou se trouve', 'localisation', 'gps', 'itineraire', 'trouver'],
        'category' => 'adresse',
        'priority' => 8
    ],
    // Restaurant / Recommandations
    [
        'keywords' => ['restaurant', 'resto', 'manger', 'diner', 'dejeuner', 'ou manger', 'bonne table', 'recommandation', 'recommander', 'conseil', 'conseiller', 'suggestion', 'activite', 'visiter', 'faire', 'sortir', 'bar', 'boire un verre'],
        'category' => 'recommandation',
        'priority' => 7
    ],
    // Transport / Taxi
    [
        'keywords' => ['taxi', 'uber', 'vtc', 'transport', 'bus', 'metro', 'tramway', 'gare', 'aeroport', 'navette'],
        'category' => 'transport',
        'priority' => 6
    ],
    // Supermarche / Commerces
    [
        'keywords' => ['supermarche', 'courses', 'magasin', 'commerce', 'boulangerie', 'pharmacie', 'epicerie', 'acheter'],
        'category' => 'commerces',
        'priority' => 5
    ],
    // Horaires / Timing
    [
        'keywords' => ['a quelle heure', 'quand', 'horaire', 'disponible', 'possible de', 'peut-on'],
        'category' => 'horaires',
        'priority' => 5
    ]
];
    ...
    switch ($category) {
        ...
        case 'checkin':
            ...
            if (!empty($parts)) {
                ...
                return [
                    'category' => 'checkin',
                    'label' => 'Arrivee (a completer)',
                    'text' => "$greeting, l'arrivee est prevue a partir de 15h. Je vous enverrai les instructions d'acces avant votre arrivee.",
                    'confidence' => 0.4
                ];
            }
        ...
        case 'parking':
            if ($ctx['parking_disponible']) {
                ...
                return [
                    'category' => 'parking',
                    'label' => 'Parking (pas dispo)',
                    'text' => "$greeting, malheureusement il n'y a pas de parking prive. Vous trouverez des places de stationnement dans les rues environnantes.",
                    'confidence' => 0.7
                ];
            }
        ...
        case 'menage':
            ...
            $items = [];
            if (!empty($eq['linge_fourni'])) {
                $items[] = "les draps et serviettes sont fournis";
            }
            if (!empty($eq['machine_a_laver'])) {
                $items[] = "une machine a laver est a disposition";
            }
        ...
        case 'cuisine':
            ...
            $items = [];
            if (!empty($eq['type_cafetiere'])) {
                $items[] = "une cafetiere " . $eq['type_cafetiere'];
            }
            if (!empty($eq['bouilloire'])) {
                $items[] = "une bouilloire";
            }
            if (!empty($eq['micro_ondes'])) {
                $items[] = "un micro-ondes";
            }
            if (!empty($eq['four'])) {
                $items[] = "un four";
            }
            if (!empty($eq['lave_vaisselle'])) {
                $items[] = "un lave-vaisselle";
            }
        ...
        case 'tv':
            ...
            if (!empty($eq['tv'])) {
                $tv_info = "une TV";
                if (!empty($eq['tv_pouces'])) {
                    $tv_info .= " " . $eq['tv_pouces'] . " pouces";
                }
                $items[] = $tv_info;
            }
            if (!empty($eq['netflix'])) {
            ...
            if (!empty($eq['amazon_prime'])) {
                $items[] = "Amazon Prime";
            }
            if (!empty($eq['disney_plus'])) {
                $items[] = "Disney+";
            }
        ...
        case 'chauffage':
            return [
                ...
                'text' => "$greeting, le chauffage/climatisation se controle via le thermostat situe dans le salon. N'hesitez pas si vous avez besoin d'aide pour le regler.",
                'confidence' => 0.6
            ];
        ...
        case 'probleme':
            return [
                'category' => 'probleme',
                'label' => 'Reponse probleme',
                'text' => "$greeting, je suis desole d'apprendre que vous rencontrez un probleme. Pouvez-vous me donner plus de details ? Je fais le necessaire au plus vite.",
                'confidence' => 0.9
            ];
        ...
        case 'remerciement':
            return [
                'category' => 'remerciement',
                'label' => 'Reponse remerciement',
                'text' => "$greeting, merci beaucoup pour votre message ! Je suis ravi que tout se passe bien. N'hesitez pas si vous avez besoin de quoi que ce soit.",
                'confidence' => 0.85
            ];
        ...
        case 'retard':
            return [
                'category' => 'retard',
                'label' => 'Reponse retard',
                'text' => "$greeting, pas de probleme pour votre retard. Tenez-moi informe de votre heure d'arrivee estimee. Bonne route !",
                'confidence' => 0.85
            ];
        ...
        case 'recommandation':
            ...
            // 1. Chercher les restaurants dans les recommandations de la ville
            ...
            // 4. Chercher les partenaires (services, guides, etc.)
            ...
            return [
                'category' => 'recommandation',
                'label' => 'Recommandations' . ($ville ? " $ville" : ""),
                'text' => $response,
        ...
        case 'transport':
            ...
            // 1. Chercher dans les recommandations de la ville
