...
function generateDbPrefix($name, $pdo) {
    $letters = strtolower(preg_replace('/[^a-zA-Z]/', '', $name));
    $letters = substr($letters, 0, 3);
    if (strlen($letters) < 2) $letters = 'fs';

    // Essayer jusqu'à trouver un préfixe unique
    for ($i = 0; $i < 20; $i++) {
        $prefix = $letters . sprintf('%02d', rand(0, 99)) . '_';
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM frenchysite_instances WHERE db_prefix = :prefix");
...
function loadLogementEquipements($pdo, $logementId) {
    try {
...
function createSiteTables($pdo, $dbPrefix, $siteName, $logementData, $equipements) {
    // Lire le schema.sql du moteur frenchysite
    ...
    // Exécuter le schéma (CREATE + INSERT defaults)
    $pdo->exec($sql);

    // ── Mettre à jour les settings avec les données du logement ──
    $stmtSetting = $pdo->prepare("UPDATE {$dbPrefix}settings SET setting_value = ? WHERE setting_key = ?");
    ...
    if (!empty($logementData['description'])) {
        // Utiliser la description du logement comme tagline
    ...
    $stmtText = $pdo->prepare("UPDATE {$dbPrefix}texts SET field_value = ? WHERE section_key = ? AND field_key = ?");
    ...
    if (!empty($equipements)) {
        ...
        // Cuisine : construire la liste des équipements
        $cuisineItems = [];
        ...
        // Cinéma / divertissement : construire la liste des contenus
        if (!empty($equipements['tv'])) {
            $tvDesc = 'TV';
            if (!empty($equipements['tv_type'])) $tvDesc .= ' ' . $equipements['tv_type'];
            if (!empty($equipements['tv_pouces'])) $tvDesc .= ' ' . $equipements['tv_pouces'] . '"';
            $streaming = [];
        ...
        // Checkin / checkout dans la description du contact
        $horaires = [];
    ...
    $stmtGuide = $pdo->prepare("INSERT IGNORE INTO {$dbPrefix}guides (slug, title, subtitle, icon_svg, is_active, sort_order) VALUES (?, ?, ?, ?, 1, ?)");
...
function dropSiteTables($pdo, $dbPrefix) {
    $tables = ['guide_blocks', 'guides', 'photos', 'texts', 'settings'];
    foreach ($tables as $t) {
        $pdo->exec("DROP TABLE IF EXISTS `{$dbPrefix}{$t}`");
    }
}

// ── Compter les tables existantes pour un préfixe ──
function countSiteTables($pdo, $dbPrefix) {
    $count = 0;
    $tables = ['settings', 'texts', 'photos', 'guides', 'guide_blocks'];
    foreach ($tables as $t) {
        try {
            $pdo->query("SELECT 1 FROM `{$dbPrefix}{$t}` LIMIT 1");
            $count++;
        } catch (PDOException $e) {
            // Table n'existe pas
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_site'])) {
    ...
    if ($logement_id <= 0) {
        ...
        // Vérifier que le logement n'a pas déjà un site
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM frenchysite_instances WHERE logement_id = :lid");
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM liste_logements WHERE id = :id");
            ...
            if (!$logement) {
                ...
                try {
                    ...
                    $dbPrefix = generateDbPrefix($siteName, $pdo);
                    $passHash = $admin_pass ? password_hash($admin_pass, PASSWORD_BCRYPT) : '';

                    // Charger les équipements du logement
                    $equipements = loadLogementEquipements($pdo, $logement_id);

                    // Créer les tables avec données du logement + équipements
                    createSiteTables($pdo, $dbPrefix, $siteName, $logement, $equipements);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_site'])) {
    ...
    try {
        if ($admin_pass) {
            ...
            $stmt = $pdo->prepare("UPDATE frenchysite_instances SET site_url = :site_url, admin_pass_hash = :pass WHERE id = :id");
            $stmt->execute([':site_url' => $site_url, ':pass' => $passHash, ':id' => $id]);
        } else {
            $stmt = $pdo->prepare("UPDATE frenchysite_instances SET site_url = :site_url WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_site'])) {
    ...
    $id = (int)$_POST['site_id'];
    try {
        $pdo->prepare("UPDATE frenchysite_instances SET actif = NOT actif WHERE id = :id")->execute([':id' => $id]);
        $stmt = $pdo->prepare("SELECT actif, site_name FROM frenchysite_instances WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $result = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_site'])) {
    ...
    $id = (int)$_POST['site_id'];
    try {
        // Récupérer le préfixe pour supprimer les tables
        $stmt = $pdo->prepare("SELECT db_prefix, site_name FROM frenchysite_instances WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $site = $stmt->fetch();

        if ($site) {
            dropSiteTables($pdo, $site['db_prefix']);
            $pdo->prepare("DELETE FROM frenchysite_instances WHERE id = :id")->execute([':id' => $id]);
...
foreach ($sites as $site) {
    $sitesHealth[$site['id']] = countSiteTables($pdo, $site['db_prefix']);
}
?>
        ...
        <div class="card shadow-custom">
            ...
            <div class="card-body">
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        <label for="admin_pass"><i class="fas fa-lock"></i> Mot de passe admin</label>
                    ...
                    <button type="submit" name="create_site" class="btn btn-success btn-block">
                            ...
                            <tbody>
                                <?php foreach ($sites as $site): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($health === 5): ?>
                                                <span class="badge badge-success"><i class="fas fa-check"></i> 5/5</span>
                                            <?php elseif ($health > 0): ?>
                                                <span class="badge badge-warning"><i class="fas fa-exclamation"></i> <?= $health ?>/5</span>
                                            <?php else: ?>
                                                <span class="badge badge-danger"><i class="fas fa-times"></i> 0/5</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label><i class="fas fa-database"></i> Préfixe BDD</label>
                        <input type="text" class="form-control" id="edit_db_prefix" disabled>
                    </div>

                    <div class="form-group">
                        <label for="edit_site_url"><i class="fas fa-link"></i> URL du site</label>
                        <input type="url" class="form-control" id="edit_site_url" name="site_url" placeholder="https://monlogement.fr">
                    </div>

                    <div class="form-group">
                        <label for="edit_admin_pass"><i class="fas fa-lock"></i> Nouveau mot de passe admin</label>
