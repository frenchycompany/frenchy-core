...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_text_module'])) {
    ...
    // Insérer le module texte unique dans client_modules
    // Attention : Vérifiez que la valeur "1" correspond bien à l'ID du module texte générique dans la table modules !
    ...
    header("Location: ?section=modules&client_id=" . $client_id);
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['toggle_module'])) {
    // Récupérer l'ID du module à basculer depuis le bouton (la valeur est envoyée dans toggle_module)
    $module_id = intval($_POST['toggle_module']);
...
<body>
    <header>
        ...
        <a href="?section=dashboard">Retour au Tableau de Bord</a>
        <a href="logout.php">Déconnexion</a>
    </header>

    <main>
        <h2>Sélectionner un Client</h2>
        <form action="?section=modules" method="get">
        ...
        <?php if ($client_id): ?>
            ...
            <form action="?section=modules&client_id=<?php echo $client_id; ?>" method="post">
                <table class="table">
                    <thead>
                        <tr>
                            <th>Module</th>
                    ...
                    <tbody>
                        <?php foreach ($modules as $module): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($module['nom'], ENT_QUOTES, 'UTF-8'); ?></td>
                                <td>
            ...
            <?php if (in_array(17, $client_modules)) : ?>
                <a href="?section=texts&client_id=<?php echo $client_id; ?>" class="btn">📝 Modifier les textes</a>
            <?php endif; ?>
            ...
            <ul id="sortable">
                <?php foreach ($client_modules_texts as $text_module): ?>
                    <li class="sortable-item" data-module-id="<?php echo $text_module['id']; ?>">
                        <?php echo htmlspecialchars($text_module['nom_module'], ENT_QUOTES, 'UTF-8'); ?>
                        <form action="?section=modules&client_id=<?php echo $client_id; ?>" method="post" style="display:inline;">
