...
// 1) Lire les DÉPARTS du jour (REMOTE)
try {
    $sqlDeps = "
...
// 2) Lire les ARRIVÉES du jour (REMOTE)
try {
    $sqlArrs = "
        SELECT
            r.id AS resa_id,
...
try {
    ...
    $rows_t = $stmtTaches->fetchAll(PDO::FETCH_ASSOC);
    $stmtTaches->closeCursor();
    foreach ($rows_t as $t) {
...
try {
    ...
    // === 1) TRAITEMENT DES DÉPARTS (ménage de sortie) ===
    foreach ($deps as $r) {
