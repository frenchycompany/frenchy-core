...
try {
    ...
    // ── 2. Prix par jour : lire depuis superhote_price_updates (RPI) = vrais prix générés ──
    $dailyPrices = []; // [logement_id][date] = prix

    // Priorité 1 : lire les prix déjà calculés par generate_prices.php sur le RPI
    if ($pdoRpi) try {
        $priceStmt = $pdoRpi->prepare("
        ...
        foreach ($priceStmt->fetchAll(PDO::FETCH_ASSOC) as $p) {
            ...
            if (!isset($logementNames[$lid])) continue; // ignorer logements inactifs
            $dailyPrices[$lid][$p['date_start']] = (float) $p['price'];
        }
    } catch (PDOException $e) {
        error_log('calendrier_api superhote_price_updates RPI: ' . $e->getMessage());
    }

    // Priorité 2 : fallback sur superhote_price_updates du VPS
    ...
    // Priorité 3 : recalcul dynamique pour les logements sans prix dans superhote_price_updates
    $logementsSansPrix = array_diff_key($logementPricing, $dailyPrices);
    if (!empty($logementsSansPrix)) {
        $superhoteSettings = [];
        if ($pdoRpi) try {
        ...
        foreach ($logementsSansPrix as $lid => $lp) {
            ...
            while ($cursor <= $dateEnd) {
                ...
                $jourSemaine = (int) $cursor->format('w');
    ...
    // Compléter $logementPricing depuis les prix si besoin
    ...
    echo json_encode([
        ...
        '_debug'       => [
            ...
            'prices_source'      => !empty($logementsSansPrix) && count($logementsSansPrix) < count($logementPricing)
                                        ? 'superhote_price_updates + fallback_calcul'
                                        : (count($dailyPrices) > 0 ? 'superhote_price_updates' : 'aucun'),
        ],
    ]);

} catch (PDOException $e) {
    error_log('calendrier_api.php: ' . $e->getMessage());
