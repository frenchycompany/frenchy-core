...
<?php if (!empty($reservations_to_display)): ?>
            ...
            <tbody>
                <?php foreach ($reservations_to_display as $res): ?>
                    ...
                    if ($res['date_depart'] < $today) {
                        ...
                        $date_badge = '<span class="badge text-bg-success ml-2">En cours</span>';
                    } elseif ($res['jours_avant_arrivee'] <= 7 && $res['jours_avant_arrivee'] > 0) {
                        $row_class = 'table-warning'; // Bientôt
                        $date_badge = '<span class="badge text-bg-warning ml-2">Dans ' . $res['jours_avant_arrivee'] . ' jour(s)</span>';
                    }
                    ?>
                    <tr class="<?= $row_class ?>">
                        <td>
    ...
    <div class="mt-3">
        <small class="text-muted">
            ...
            <span class="badge text-bg-success ml-2">En cours</span>
            <span class="badge text-bg-warning ml-2">Arrivée dans 7 jours</span>
            <span class="badge text-bg-secondary ml-2">Passée</span>
        </small>
    </div>
<?php else: ?>
